...
public class LogScripCanceller implements Listener {
    ...
    public void LogScripEvent(PlayerInteractEvent event){
        ...
        if (!event.getClickedBlock().getType().equals(Material.ACACIA_LOG) ||
                ...
                !event.getClickedBlock().getType().equals(Material.SPRUCE_LOG)) return;
        ...
        if (!event.getPlayer().getInventory().getItemInMainHand().getType().equals(Material.IRON_AXE) ||
                ...
                !event.getPlayer().getInventory().getItemInMainHand().getType().equals(Material.NETHERITE_AXE)) return;
